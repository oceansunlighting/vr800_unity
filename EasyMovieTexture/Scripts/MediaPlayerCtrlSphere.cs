...
public class MediaPlayerCtrlSphere : MonoBehaviour {

	private static MediaPlayerCtrlSphere m_instance;
	...
	private float m_ftime=-99;
	private string[] arrstring = new string[]{ "Play","Pause","Stop"};
	private Dictionary<GameObject,string> m_Dic = new Dictionary<GameObject, string> (); 
	...
	private bool m_bFireone = false;
	private bool m_bFireTwo = false;
	private bool m_bFireThree = false;
	private bool m_bFireJump = false;

	private float m_fFireOneTime = -99;
	private float m_fFireTwoTime = -99;
	private float m_fFireThreeTime = -99;
	private float m_fFireFourTime = -99;

	private float m_fTime =-99;

	private Vector3 VecPos = Vector3.zero;
    private bool bSetValue = false;
    private float fGetTimeScale = 0;
    private float m_fSpecialTime = -99;
	...
	void Start () 
	{
		...
		#endif
		m_bInit = true;
		//VecPos = go_PlayMovie.transform.localPosition;
	...
	void Update()
	{
        ...
        if (bSetValue)
        {
            if (Time.realtimeSinceStartup - m_fSpecialTime >= 4f)
            {
                SetRecentData();
            }
        }
...
{
	StartCoroutine(DownloadStreamingVideoAndLoad2 (MovieName));
}
public IEnumerator DownloadStreamingVideoAndLoad2(string strURL)
{
	...
	public static MediaPlayerCtrlSphere f_instance
	{
		get
		{
				return m_instance;	
    ...
    public float f_fGetTimeScale
    {
        get
        {
            return fGetTimeScale;
        }
        set
        {
            fGetTimeScale = value;
    ...
    public float f_SpecialTime
    {
        get
        {
            return m_fSpecialTime;
        }
        set
        {
            m_fSpecialTime = value;
        }
    }
